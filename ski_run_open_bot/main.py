...
def monitor_for_run_status_change_and_take_action(scrape_function: Callable, action_function: Callable, action_args=[], check_period_minute:int=5):
    ...
    try:
        resort_data = ski_run.load_resort_run_data_from_file(resort_data.resort)
    except FileNotFoundError:
        logger.info(f'No file found for resort {new_data.resort}. File will be created.')
        
    
    while 1:
        # load the ski runs
        new_data = scrape_function()
        
        if resort_data is not None: # this if statement should skip the check 
            recently_opened_runs = ski_run.filter_for_opened_runs(new_data.run_data, resort_data.run_data)
            action_function(recently_opened_runs, *action_args)
                            
        resort_data = new_data['ski_runs']
